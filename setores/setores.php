...
if(isset($_SESSION['id_usuario']) && empty($_SESSION['id_usuario'])==false){
    ...
    $selecionar = $db->query("SELECT * FROM setor");
            ...
            <div class="tela-principal">
                ...
                <div class="menu-principal">
                    <div class="table-responsive">
                        <table class="table table-striped table-dark table-bordered"> 
                            <thead>
                                <tr>
                                    <th scope="col" class="text-center text-nowrap">ID Setor</th>
                                    <th scope="col" class="text-center text-nowrap">Nome Setor</th>
                            ...
                            <tbody>
                                ...
                                $totalSetor = $selecionar->rowCount();
                                $qtdPorPagina = 10;
                                $numPaginas = ceil($totalSetor/$qtdPorPagina);
                                $paginaInicial = ($qtdPorPagina*$pagina)-$qtdPorPagina;
                                $limitado = $db->query("SELECT * FROM setor LIMIT $paginaInicial,$qtdPorPagina ");
                                
                                if($limitado->rowCount()>0){
                                    $dados = $limitado->fetchAll();
                                    foreach($dados as $dado){
                                ?>
                                <tr id="<?php echo $dado['idsetor'] ?>">
                                    <td scope="col" class="text-center text-nowrap"> <?php echo $dado['idsetor']; ?> </td>
                                    <td scope="col" class="text-center text-nowrap"> <?php echo $dado['nome_setor']; ?> </td>
                                    <td scope="col" class="text-center text-nowrap">
                                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal<?php echo $dado['idsetor']; ?>" data-whatever="@mdo" value="<?php echo $dado['idsetor']; ?>" name="idsetor" >Visualisar</button>
                                    </td>
                                </tr>
                                <!-- INICIO MODAL visualisar ssetor-->
                                <div class="modal fade" id="modal<?php echo $dado['idsetor']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                ...
                                                <form action="atualiza.php" method="post">
                                                    <div class="form-row">
                                                        <div class="form-group col-md-12">
                                                            <label for="idsetor" class="col-form-label">ID</label>
                                                            <input type="text" name="idsetor" class="form-control" readonly id="idsetor" value="<?php echo $dado['idsetor'] ?>">
                                                        </div>
                                                    </div>
                                                    <div class="form-row">
                                                        <div class="form-group col-md-12">
                                                            <label for="nome_Setor" class="col-form-label">Nome Setor</label>
                                                            <input type="text" name="nome_setor" class="form-control"  id="nome_setor" value="<?php echo $dado['nome_setor'];  ?>">
                                                        </div>
                                                    </div>      
                                            </div>
                                            <div class="modal-footer">
                                                    <a href="excluir.php?idsetor=<?php echo $dado['idsetor']; ?>" class="btn btn-danger" > Excluir </a>
                ...
                <nav aria-label="Navegação de página exemplo" class="paginacao">
                    <ul class="pagination">
                        ...
                        <?php
                            if($paginaAnterior!=0){
                                echo "<a class='page-link' href='setores.php?pagina=$paginaAnterior' aria-label='Anterior'>
                        ...
                        <?php
                            for($i=1;$i < $numPaginas+1;$i++){
                                echo "<li class='page-item'><a class='page-link' href='setores.php?pagina=$i'>$i</a></li>";
                            }
                        ?>
                        <li class="page-item">
                        <?php
                            if($paginaPosterior <= $numPaginas){
                                echo " <a class='page-link' href='setores.php?pagina=$paginaPosterior' aria-label='Próximo'>
